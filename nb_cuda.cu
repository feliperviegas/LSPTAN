...
__global__ void super_parent_best_child(double *probSp, double *matrixTermFreq,
	...
	if (idx < numTerms) {
		for (c = 0; c < numClasses; c++) {
			...
			if (c == 0) {
				maiorProb = sumProb;
			} else if (sumProb > maiorProb) {
				maiorProb = sumProb;
			}
		}
		probChildSp[idx] = maiorProb;
	}
}


/*
__host__ void super_parent_update(double *probSp, double *matrixTermFreq,
		double *totalFreqClassVector, int *docTestIndexVector,
	...
	for (c = 0; c < numClasses; c++) {
		...
		for (t = inicio; t < fim; t++) {
			...
			if (term == filho) {
				...
				nt = freqTermVector[term] / (double) totalTermFreq;
				prob = log(lambda * nt + (1.0 - lambda) * probSp[c * numTerms + term]) - log(lambda * nt + (1.0 - lambda) * prob);
				sumProb = sumProb + freq * prob;
			}
		}
...
/*
Dúvidas:
	Quando divisão não retorna inteiro.
		-ceil arredonda para zero. Não Entra no loop.
...
__global__ void super_parent_update(double *probSp, double *matrixTermFreq,
	...
	if (tid == 0) {  // thread 0 calculates vecs and len
		//vecs - vector size
		aux[blockDim.x + 1] = (docTestIndexVector[d + 1]
				- docTestIndexVector[d]);
		// len - number of segments (size nthreads) of the vector
		if(aux[blockDim.x + 1] > blockDim.x)
			aux[blockDim.x + 2] = ceil(aux[blockDim.x + 1] / (float) blockDim.x);
		else
	...
	for (int c = 0; c < numClasses; c++) {
		for (int b = 0; b < len; b++) { // loop through 'len' segments
			// first, each thread loads data into shared memory
			if ((b * blockDim.x + tid) < vecs){ // check if outside 'vec' boundary
				term = docTestVector[docTestIndexVector[d]
						+ b * blockDim.x + tid];
				freq = docTestFreqVector[docTestIndexVector[d]
						+ b * blockDim.x + tid];
				prob = (matrixTermFreq[c * numTerms + term] + alpha)
						/ (totalFreqClassVector[c] + alpha * totalTerms);
				nt = freqTermVector[term] / totalTermFreq;
...
//        cudaMemcpy(probMatrix, probMatrix_D, sizeof(double)*numClasses*numTerms, cudaMemcpyDeviceToHost);
	for (int d = 0; d < numDocsTest; d++) {
		probAux[d] = -9999999.0;
		superParents[d] = -1;
	}
...
//            super_parent_predict<<< n_blocks, block_size >>>(probSp_D, probMatrix_D, docTestIndexVector_D, docTestVector_D, docTestFreqVector_D, probClassSp_D, numClasses, numTerms, numDocsTest, freqTermVector_D, totalTermFreq, totalTerms, sp, modeloNB_D, lambda);
		...
		super_parent_predict2<<<n_blocks, block_size,
				...
				lambda, alpha, sp, modeloNB_D, probSp_D, docClassSp_D);
		//cudaDeviceSynchronize();
	...
	cerr << "Melhor SP " << evaluate(realClass, predictClass, numDocsTest, 1) * 100 << " "
			<< evaluate(realClass, predictClass, numDocsTest, 0) * 100 << " time: " << double(endT - begin) << endl;
	cout << "Melhor SP " << evaluate(realClass, predictClass, numDocsTest, 1) * 100 << " "
			<< evaluate(realClass, predictClass, numDocsTest, 0) * 100 << " time: " << double(endT - begin) << endl;

	for (int d = 0; d < numDocsTest; d++) {
		if (superParents[d] != -1) {
			//Constroi a tabela de probabilidades para o melhor Super Pai selecionado
			//block_size = 384;
			//n_blocks = (numTerms + 1) / block_size
			//		+ ((numTerms + 1) % block_size == 0 ? 0 : 1);
			//super_parent_train<<<n_blocks, block_size>>>(docIndexVector_D,
			...
			//		numClasses, probSp_D, superParents[d], alpha);
			block_size = SIZE_TRAIN;
			n_blocks = numDocs;
			find_sp_kernel<<<n_blocks, block_size>>>(docIndexVector_D, docVector_D, docFreqVector_D,
				numClasses, numTerms, numDocs, totalTerms, hasSp_D, superParents[d]);
			...
			super_parent_train<<<n_blocks, block_size>>>(docIndexVector_D,
				docVector_D, docFreqVector_D, docClassVector_D,
				totalTermClassSp_D, numTerms, numDocs, totalTerms, numClasses,
				probSp_D, superParents[d], alpha, hasSp_D);

			cudaMemcpy(probSp, probSp_D, sizeof(double) * numClasses * numTerms,
					cudaMemcpyDeviceToHost);

			//Classificação dos documentos considerando cada filho individualmente dado o melhor Super Pai selecionado
			block_size = 384;
			n_blocks = (numTerms + 1) / block_size
					+ ((numTerms + 1) % block_size == 0 ? 0 : 1);
			super_parent_best_child<<<n_blocks, block_size>>>(probSp_D,
					...
					docTestIndexVector_D, docTestVector_D, docTestFreqVector_D,
					probChildSp_D, numClasses, numTerms, numDocsTest,
					freqTermVector_D, totalTermFreq, totalTerms,
					superParents[d], modeloNB_D, d, lambda, alpha);
			cudaMemcpy(probChildSp, probChildSp_D, sizeof(double)*numTerms, cudaMemcpyDeviceToHost);
			cerr << d << " " << probAux[d] << " " << probClasse[d] << endl;
			for (int t = 0; t < numTerms; t++) {
				if (superParents[d] != t && probClasse[d] < probChildSp[t]) { //probAux -> probabilidade gerada pelo melhorSP
					//Atualiza o modelo de classificação
					//super_parent_update(probSp, matrixTermFreq, totalFreqClassVector, docTestIndexVector, docTestVector, docTestFreqVector, numClasses, numTerms, numDocsTest, freqTermVector, totalTermFreq, totalTerms, t, modeloNB, d, lambda, alpha);
					block_size = 384;
					n_blocks = (numTerms + 1) / block_size
					+ ((numTerms + 1) % block_size == 0 ? 0 : 1);
					super_parent_update<<<n_blocks, block_size>>>(probSp_D, matrixTermFreq_D, totalFreqClassVector_D, docTestIndexVector_D,
						docTestVector_D, docTestFreqVector_D, numClasses, numTerms, numDocsTest, freqTermVector_D, totalTermFreq, totalTerms,
						t, modeloNB_D, d, lambda, alpha);

				}
			}
			//cudaMemcpy(modeloNB_D, modeloNB,
			//		sizeof(double) * (numClasses * (numDocsTest)),
			//		cudaMemcpyHostToDevice);
		}
	}
	cudaMemcpy(modeloNB, modeloNB_D, sizeof(double)*numDocsTest*numClasses, cudaMemcpyDeviceToHost);
	for (int d = 0; d < numDocsTest; d++) {
		maiorProb = -9999999.0;
		for (int c = 0; c < numClasses; c++) {
			if (modeloNB[d * numClasses + c] > maiorProb) {
	...
	cerr << "Filhos Favoritos " << evaluate(realClass, predictClass, numDocsTest, 1)*100 << " " << evaluate(realClass, predictClass, numDocsTest, 0)*100 << " ";

	//fTreino = tempoAtual();

	cout << "Filhos Favoritos " << evaluate(realClass, predictClass, numDocsTest, 1)*100 << " " << evaluate(realClass, predictClass, numDocsTest, 0)*100 << " ";
